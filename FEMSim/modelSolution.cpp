...
QVariant ModelSolution::headerData(int section, Qt::Orientation orientation, int role) const
{
    QVariant ret;
    ...
    //log0("[%s] section = %s, ret = %d", __FUNCTION__, ret, section);
    return ret;
}
    ...
    switch (index.column())
    {
        ...
        case COLUMN1:
        {
            if (domElement.hasAttribute("GLenum"))
            {
                QString str;
                str.append("");
                switch (domElement.attribute("GLenum").toInt())
                {
                    case GL_POINTS:
                    {
						str.append("GL_POINTS");
						str.append(",");
